...
export const useVideoGeneration = () => {
  ...
  const generateVideo = useCallback(
    async (imageUrl: string, prompt: string | undefined) => {
      setLoading(true);
      setError("");
        ...
        const pollTaskStatus = async () => {
          try {
            ...
            if (taskStatus === "succeed") {
              setVideoUrl(
                queryResponse.data.task_result?.videos?.[0]?.url || null
              );
              setLoading(false);
            } else if (taskStatus === "failed") {
              setError(
                queryResponse.data.task_status_msg || "Video generation failed."
              );
              setLoading(false);
            } else {
            ...
            console.error("Error querying task status. Please try again.", error);
